...
import 'package:clinic_dr_alla/Pages/beauty_centers_page/beauty_center_details_page/beauty_center_details_page.dart';
...
class BeautyCentersPage extends StatefulWidget {
  @override
  _BeautyCentersPageState createState() => _BeautyCentersPageState();
}

class _BeautyCentersPageState extends State<BeautyCentersPage> {
  final ScrollController _scrollController = ScrollController();
  List<BeautyCenterModel> _pharmacies = [];
  int _currentPage = 1;
  int _lastPage = 1;
  ...
  Future<void> fetchPharmacies() async {
    ...
    try {
      ...
      final response = await http.get(Uri.parse('$urlBeautyCenters$query'));
      ...
      if (response.statusCode == 200) {
        ...
        final List<BeautyCenterModel> fetched =
            data.map((e) => BeautyCenterModel.fromJson(e)).toList();
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      appBar: MyCustomAppBar(name: "مراكز تجميل"),
                      ...
                      return InkWell(
                        onTap: () {
                          Navigator.push(
                            context,
                            MaterialPageRoute(
                              builder: (_) => BeautyCenterDetailsPage(
                                  beautyCenter: pharmacy),
                            ),
                          );
                        },
                        child: Card(
