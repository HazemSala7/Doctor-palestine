...
import 'package:clinic_dr_alla/Pages/radiology_centers_page/radiology_center_details_page/radiology_center_details_page.dart';
...
class RadiologyCentersPage extends StatefulWidget {
  @override
  _RadiologyCentersPageState createState() => _RadiologyCentersPageState();
}

class _RadiologyCentersPageState extends State<RadiologyCentersPage> {
  final ScrollController _scrollController = ScrollController();
  List<RadiologyCenterModel> _pharmacies = [];
  int _currentPage = 1;
  int _lastPage = 1;
  ...
  Future<void> fetchPharmacies() async {
    ...
    try {
      ...
      final response = await http.get(Uri.parse('$urlRadiologyCenters$query'));
      ...
      if (response.statusCode == 200) {
        ...
        final List<RadiologyCenterModel> fetched =
            data.map((e) => RadiologyCenterModel.fromJson(e)).toList();
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      appBar: MyCustomAppBar(name: "مراكز أشعة"),
                      ...
                      return InkWell(
                        onTap: () {
                          Navigator.push(
                            context,
                            MaterialPageRoute(
                              builder: (_) => RadiologyCenterDetailsPage(
                                  radiologyCenter: pharmacy),
                            ),
                          );
                        },
                        child: Card(
